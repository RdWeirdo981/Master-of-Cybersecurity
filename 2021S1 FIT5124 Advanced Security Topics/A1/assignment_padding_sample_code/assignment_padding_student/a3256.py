...
for id_set in inverted_dict.values():
    ...
    for i in range(len(cluster_points_256)-1):
        highest = id_list[cluster_points_256[i]]
        lowest = id_list[cluster_points_256[i+1]-1]
...
# for i in range(len(cluster_points_256)-1):
    # for j in range (cluster_points_256[i], cluster_points_256[i+1]): # e.g. 392 - 647
        # diff = id_list[i] - id_list[j]
        # for k in range(diff):
            # id_list.append(k)
            # count_padding += 1
            # print("added!",count_padding)
